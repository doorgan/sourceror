-module(sourceror_elixir_tokenizer).
...
tokenize([$~, S, H | T] = Original, Line, Column, Scope, Tokens) when ?is_sigil(H), ?is_upcase(S) orelse ?is_downcase(S) ->
  case sourceror_elixir_interpolation:extract(Line, Column + 3, Scope, ?is_downcase(S), T, sigil_terminator(H)) of
...
tokenize([$?, $\\, H | T], Line, Column, Scope, Tokens) ->
  Char = sourceror_elixir_interpolation:unescape_map(H),
...
tokenize([$:, H | T] = Original, Line, Column, Scope, Tokens) when ?is_quote(H) ->
  case sourceror_elixir_interpolation:extract(Line, Column + 2, Scope, true, T, H) of
    {NewLine, NewColumn, Parts, Rest} ->
...
handle_strings(T, Line, Column, H, Scope, Tokens) ->
  case sourceror_elixir_interpolation:extract(Line, Column, Scope, true, T, H) of
    {error, Reason} ->
...
handle_dot([$., H | T] = Original, Line, Column, DotInfo, Scope, Tokens) when ?is_quote(H) ->
  case sourceror_elixir_interpolation:extract(Line, Column + 1, Scope, true, T, H) of
...
extract_heredoc_with_interpolation(Line, Column, Scope, Interpol, T, H) ->
  case extract_heredoc_header(T) of
    {ok, Headerless} ->
      ...
      case sourceror_elixir_interpolation:extract(Line, Column, Scope, Interpol, [$\n|Headerless], [H,H,H]) of
...
unescape_tokens(Tokens, Line, Column, #elixir_tokenizer{unescape=true}) ->
  case sourceror_elixir_interpolation:unescape_tokens(Tokens) of
    {ok, Result} ->
      {ok, Result};
