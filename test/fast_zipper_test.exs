defmodule SourcerorTest.FastZipperTest do
  use ExUnit.Case, async: true

  # Unimporting Kernel to avoid conflicts with the `node/1` function
  import Kernel, except: [node: 1]
  import SourcerorTest.CursorSupport, only: [pop_cursor: 1, pop_range: 1]

  import Sourceror.FastZipper, only: :macros

  alias Sourceror.FastZipper, as: Z

  doctest Sourceror.FastZipper, import: true

  describe "zip/1" do
    test "creates a zipper from a term" do
      assert zipper(node: 42) = Z.zip(42)
    end
  end
  ...
  describe "up/1" do
    test "reconstructs the previous parent" do
      assert Z.zip([1, 2]) |> Z.down() |> Z.up() == zipper(node: [1, 2])
      assert Z.zip({1, 2}) |> Z.down() |> Z.up() == zipper(node: {1, 2})
      assert Z.zip({:foo, [], [1, 2]}) |> Z.down() |> Z.up() == zipper(node: {:foo, [], [1, 2]})
  ...
  describe "traverse/2" do
    test "traverses in depth-first pre-order" do
      ...
      assert Z.traverse(zipper, fn
               zipper(node: x) = z when is_integer(x) -> zipper(z, node: x * 2)
               z -> z
             end)
    ...
    test "traverses a subtree in depth-first pre-order" do
      ...
      assert zipper
             ...
             |> Z.right()
             |> Z.traverse(fn
               zipper(node: x) = z when is_integer(x) -> zipper(z, node: x + 10)
               z -> z
             end)
  ...
  describe "traverse_while/2" do
    test "traverses in depth-first pre-order and skips branch" do
      ...
      assert zipper
             |> Z.traverse_while(fn
               zipper(node: [x | _]) = z when rem(x, 2) != 0 -> {:skip, z}
               zipper(node: [_ | _]) = z -> {:cont, z}
               zipper(node: x) = z -> {:cont, zipper(z, node: x + 100)}
             end)
    ...
    test "traverses in depth-first pre-order and halts on halt" do
      ...
      assert zipper
             |> Z.traverse_while(fn
               zipper(node: [x | _]) = z when rem(x, 2) != 0 -> {:halt, z}
               zipper(node: [_ | _]) = z -> {:cont, z}
               zipper(node: x) = z -> {:cont, zipper(z, node: x + 100)}
             end)
    ...
    test "traverses until end while always skip" do
      assert zipper(path: nil) = [1] |> Z.zip() |> Z.traverse_while(fn z -> {:skip, z} end)
    end
  ...
  describe "traverse_while/3" do
    test "traverses in depth-first pre-order and skips branch" do
      ...
      {_zipper, acc} =
        zipper
        |> Z.traverse_while([], fn
          zipper(node: [x | _]) = z, acc when rem(x, 2) != 0 -> {:skip, z, acc}
          zipper(node: [_ | _]) = z, acc -> {:cont, z, acc}
          zipper(node: x) = z, acc -> {:cont, z, [x + 100 | acc]}
        end)
    ...
    test "traverses in depth-first pre-order and halts on halt" do
      ...
      {_zipper, acc} =
        zipper
        |> Z.traverse_while([], fn
          zipper(node: [x | _]) = z, acc when rem(x, 2) != 0 -> {:halt, z, acc}
          zipper(node: [_ | _]) = z, acc -> {:cont, z, acc}
          zipper(node: x) = z, acc -> {:cont, z, [x + 100 | acc]}
        end)
    ...
    test "removes a node during traversal" do
      {updated, _acc} =
        ...
        |> Z.traverse_while(nil, fn
          zipper(node: 2) = z, acc -> {:remove, z, acc}
          z, acc -> {:cont, z, acc}
        end)
  ...
  describe "top/1" do
    test "returns the top zipper" do
      ...
      assert 42 |> Z.zip() |> Z.top() |> Z.top() |> Z.top() == zipper(node: 42, path: nil)
    end
  end
  ...
  describe "within/2" do
    ...
    test "uses any modified supertree" do
      ...
      updated =
        ...
        |> Z.within(fn zipper ->
          supertree = zipper(zipper, :supertree)
          upwards = Z.up(supertree)
          {:__block__, _, code} = Z.node(upwards)
          ...
          updated =
            ...
            |> Z.replace(
              {:__block__, [],
               List.insert_at(
                 code,
                 Enum.count(supertree |> zipper(:path) |> path(:left) || []) + 1,
                 :new_code
               )}
            )
            |> Z.find(&(&1 == Z.node(zipper)))

          zipper
          |> zipper(supertree: updated)
          |> Z.find(&match?({{:__block__, _, [:key]}, _value}, &1))
          |> Z.update(fn {key, _value} -> {key, {:__block__, [], [:changed]}} end)
        end)
        |> Z.root()
  ...
  describe "search_pattern/2 with cursor" do
    ...
    test "continues past current zipper focus" do
      ...
      assert [[:foo], :bar] = Z.node(code)

      assert zipper(node: :baz) = Z.search_pattern(code, ":baz")
    end
  ...
  describe "at_range/2" do
    ...
    test "creates a zipper focused on an inner literal equal to the given range" do
      assert zipper(node: {:__block__, _, [2]}) =
               zipper_at_range("""
               def foo do
                 [1, «2», 3, 4, 5]
               end
    ...
    test "creates a zipper focused on the last container node even if range is not a real node" do
      assert zipper(node: {:foo, _, [{:arg, _, nil}]}) =
               zipper_at_range("""
               def «foo»(arg) do
                 arg
               end
    ...
    test "creates a zipper focused on an alias segment" do
      assert zipper(node: {:__aliases__, _, [:Baz]}) =
               zipper_at_range("""
               alias Foo.{Bar, «Baz»}
               """)
    end
    ...
    test "creates a zipper focused on a function call name" do
      assert zipper(node: {:bar, _, [_, _, _]}) =
               zipper_at_range("""
               def foo do
                 «bar»(1, 2, 3)
               end
    ...
    test "creates a zipper focused on a qualified call when the range covers the call name" do
      assert zipper(node: {:., _, [{:__aliases__, _, [:Foo]}, :bar]}) =
               zipper_at_range("""
               def foo do
                 Foo.«bar»(1, 2, 3)
               end
    ...
    test "creates a zipper focused on a qualified call alias when the range covers the alias" do
      assert zipper(node: {:__aliases__, _, [:Foo]}) =
               zipper_at_range("""
               def foo do
                 «Foo».bar(1, 2, 3)
               end
    ...
    test "creates a zipper focused on a qualified call suffix" do
      assert zipper(node: {:., _, [{:baz, _, _}, :bar]}) =
               zipper_at_range("""
               def foo do
                 «baz.bar»(1, 2, 3)
               end
    ...
    test "creates a zipper focused on a map key usage" do
      assert zipper(node: {{:., _, [{:baz, _, _}, :bar]}, _, []}) =
               zipper_at_range("""
               def foo do
                 «baz.bar»
               end
    ...
    test "creates a zipper focused on the entire qualified call" do
      assert zipper(node: {{:., _, [{:__aliases__, _, [:Foo]}, :baz]}, _, [_, _, _]}) =
               zipper_at_range("""
               def foo do
                 «Foo.baz(1, 2, 3)»
               end
    ...
    test "creates a zipper focused on an operand of a binary operator" do
      ...
      assert zipper(node: {:__block__, _, [1]}) =
               zipper_at_range("""
               def foo do
                 «1» + 2
               end
